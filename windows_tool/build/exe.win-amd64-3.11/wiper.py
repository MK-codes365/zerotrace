...
# Windows API Constants (for fallback)
...
class NativeCppWiper:
    ...
    def run_wipe(self, method_name, progress_callback=None):
        ...
        # If wiping a volume, try to dismount it first
        if ':\\' in self.device_id:  # It's a volume path like \\.\F:
        ...
        if result != 0:
            # Non-zero means error (Windows error code)
...
class CtypesWiper:
    """Python ctypes fallback (same as current implementation)"""
...
def Wiper(device_id, buffer_size=4*1024*1024):
    """Factory function - returns best available Wiper implementation"""
